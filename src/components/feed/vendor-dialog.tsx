...
function VerifiedBadgeInfo() {
  return (
    <Popover>
      ...
      <PopoverContent align="start" className="w-72 p-3 text-xs text-[#4B5563]">
        ...
        <p>
          Everything in Verified, plus the store location has been inspected by
          Swappr — buyers can pick up orders directly from this vendor.
        </p>
      </PopoverContent>
    </Popover>
  );
  ...
  return (
    <Dialog>
      <DialogTrigger asChild>
        {/* stopPropagation only on the trigger itself, so the click that
            opens the dialog doesn't also bubble into the card's
            onClick-to-navigate handler. The portaled overlay/content are
            left untouched so Radix's own outside-click dismissal isn't
            interfered with (this was previously wrapped around the whole
            Dialog, which broke tap-to-close on mobile). */}
        <button
        ...
        >
          ...
          {seller.inspectionVerified ? (
            <Icons.shieldCheck size={12} className="shrink-0 text-amber-500" />
          ) : (
            seller.verified && (
      ...
      >
        ...
        <div className="mt-4 border-t border-[#F3F4F6]">
          ...
          {storePhotos.length > 0 ? (
            ...
            <p className="px-4 pb-4 text-sm text-[#9CA3AF]">No photos yet</p>
          )}
        </div>
      </DialogContent>
    </Dialog>
